...
import DeleteEvent from "@/app/ui/DeleteButtonForEvents";
import MasterCheckbox from "@/app/ui/MasterCheckbox";
import BulkDeleteButtonForEvents from "@/app/ui/BulkDeleteButtonForEvents";
...
export default async function EventsPage({
  ...
  if (keyword) {
    andClauses.push({
      OR: [
        ...
        { eventtype: { EventTypeName: { contains: keyword } } },
      ],
    });
  }
  if (venue) andClauses.push({ Location: { contains: venue } });
  if (type) andClauses.push({ EventTypeID: Number(type) });
  if (status) andClauses.push({ Status: status });
  if (cancelled === "yes") andClauses.push({ IsCancelled: true });
  else if (cancelled === "no") andClauses.push({ IsCancelled: false });
  ...
  if (staff) {
    andClauses.push({
      eventmember: { some: { StaffID: Number(staff) } },
    });
  }
  ...
  // ── Counts ────────────────────────────────────────────────────────────────
  const [totalAll, scheduledCount, completedCount, cancelledCount, filteredTotal, eventTypes] = await Promise.all([
    prisma.events.count(),
    ...
    prisma.eventtype.findMany({ orderBy: { EventTypeName: "asc" } }),
  ]);

  // ── Pagination ────────────────────────────────────────────────────────────
  const hardLimit = limit !== "all" ? Number(limit) : null;
  const effectiveTotal = hardLimit
    ? Math.min(hardLimit, filteredTotal)
    : filteredTotal;
  ...
  const rows = await prisma.events.findMany({
    ...
    include: {
      eventtype: true,
      eventmember: { include: { staff: true } },
    },
  ...
  const eventColumns = [
    { header: "ID", key: "EventID" },
    { header: "Title", key: "EventDescription" },
    { header: "Type", key: "eventtype.EventTypeName" },
    { header: "Location", key: "Location" },
    { header: "Date", key: "EventDate" },
    { header: "Time", key: "EventTime" },
    { header: "Status", key: "Status" },
    { header: "Members", key: "membersList" },
  ...
  const exportRows = rows.map(r => ({
    ...
    membersList: r.eventmember?.map(em => em.staff?.StaffName).join(", ") || "",
      ...
      <div className="mi" key={JSON.stringify(searchParams)}>
        ...
        <div>
          ...
          <div className="hrow">
            <h1 className="ptitle">
              Event Management
              <span>Oversee, search, and manage all recorded events</span>
            </h1>
            <Link href="/dashboard/admin/events/add" className="btn-add">
              <Plus size={16} />
              Add New Event
            </Link>
          </div>
        ...
        <div className="fc">
          ...
          <form method="GET">
            <div className="fb">
              ...
              <div className="fg">
                <label className="flabel">Event Type</label>
                <select name="type" defaultValue={type} className="fctl">
                  <option value="">All Types</option>
                  {eventTypes.map((t: { EventTypeID: number; EventTypeName: string }) => (
                    <option key={t.EventTypeID} value={String(t.EventTypeID)}>
                      {t.EventTypeName}
                    </option>
                  ))}
                </select>
            ...
            <div className="fa">
              <div className="abg">
                ...
                <Link href="/dashboard/admin/events" className="btn-reset">
                  <RotateCcw size={14} /> Reset
                </Link>
                <ExportExcelButton
                  data={exportRows}
                  columns={eventColumns}
                  fileName="events_comprehensive_data"
                />
              </div>
        ...
        <div className="tc">
                    ...
                    <tr key={e.EventID}>
                      ...
                      <td>
                        <div className="ri">
                          ...
                          <div>
                            <div className="rt">
                              {e.EventDescription || "Untitled Event"}
                            </div>
                            <div className="rm">
                              <span className="mi-item">
                                <Clock size={11} />
                                {fmtDate(e.EventDate as Date | null)}
                              </span>
                              <span className="mi-item">
                                <MapPin size={11} />
                            ...
                            {e.eventmember && e.eventmember.length > 0 && (
                              <div
                                className="rm"
                      ...
                      <td className="c">
                        {e.Status ? (
                          <span className={`sbadge ${statusClass(e.Status)}`}>
                            {e.Status}
                          </span>
                        ) : (
                      ...
                      <td className="c">
                        {e.DocumentPath ? (
                          <a
                            href={e.DocumentPath.startsWith('http') ? e.DocumentPath : `/uploads/event_docs/${e.DocumentPath.split(/[\\/]/).pop()}`}
                            download
                            target="_blank"
                      ...
                      <td className="r">
                        <div className="ar">
                          <Link
                            href={`/dashboard/admin/events/${e.EventID}`}
                            className="bti view"
                            title="View"
                          >
                            <Eye size={17} />
                          ...
                          <div className="adiv" />
                          <Link
                            href={`/dashboard/admin/events/edit/${e.EventID}`}
                            className="bti edit"
                            title="Edit"
                          >
                            <Edit size={17} />
          ...
          <div className="tf">
            <p className="pi">
              Page {safePage} of {totalPages} &nbsp;·&nbsp; {effectiveTotal} total
              records
            </p>
