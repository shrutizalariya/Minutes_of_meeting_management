...
async function GetEventById({ params }: { params: Promise<{ id: number }> }) {
      ...
      <div className="w-full max-w-lg bg-white rounded-3xl shadow-[0_10px_40px_-10px_rgba(0,0,0,0.08)] border border-slate-100 overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-50 flex items-center justify-between bg-white">
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-lg ${data.IsCancelled
          ...
          <div
            className={`flex items-center gap-1.5 px-3 py-1 rounded-full border ${data.IsCancelled
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-1.5 ml-1">
            {data.DocumentPath ? (
              <a
                href={data.DocumentPath.startsWith('http') ? data.DocumentPath : `/uploads/event_docs/${data.DocumentPath.split(/[\\/]/).pop()}`}
                target="_blank"
              ...
              >
                ...
                <ArrowLeft
                  size={14}
                  className="rotate-180 text-slate-300 group-hover:text-blue-400"
                />
              </a>
            ) : (
              <p className="text-xs text-slate-400 italic bg-slate-50/50 p-3 rounded-xl border border-dashed">
          ...
          {data.IsCancelled && (
            <div className="p-4 bg-red-50/50 border border-red-100 rounded-2xl space-y-2">
              ...
              <p className="text-xs text-red-700 font-medium leading-relaxed">
                ...
                {data.CancellationReason || "No reason specified"}
              </p>
